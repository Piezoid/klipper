...
class Fan:
    ...
    def set_speed(self, print_time, value):
        ...
        kickstart_base = max(0.1, self.off_below)
        kick_start_time = self.kick_start_time * max(0, value - self.last_fan_value) / kickstart_base
        if 10*kick_start_time > self.kick_start_time:
            # Run fan at full speed for specified kick_start_time
            self.mcu_fan.set_pwm(print_time, self.max_power)
            print_time += kick_start_time
